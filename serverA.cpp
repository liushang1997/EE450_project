...
int compute(string country, int id) {
    ...
    if (relation[country].size() == relation[country][id].size()+1) {
        return -3;  // recommend None, since it has connected to all countries
    }
    cout << "The server A is searching possible friends for User" << id << " ...\n";
    int res = 0, maxVal = -1;
...
void UDPFromMainServer() {
    ...
    servaddr.sin_port = htons(UDP_BACKA_PORT); 
    ...
    while (1) {
        // if (!fork()) {
            ...
            if (request == "askmap") {
                ...
                cout << "The server A has received request for finding possible friends of User" << id << " in " << country << '\n';
                string res = to_string(compute(country, id));
                strcpy(resBuf, res.c_str());
                sendto(sockfd, (const char *)resBuf, strlen(resBuf), MSG_CONFIRM, (const struct sockaddr *) &cliaddr, len);
                if (res == "-1") {
                    cout << "The server A has sent \"User" << id <<  " not found\" to MainServer\n";
                } else {
                    cout << "The server A has sent the result to Main Server\n";
                }
            }
        // }
    }
...
int main() { 
    loadData();
    cout << "The server A is up and running using UDP on port " << UDP_BACKA_PORT << '\n';
    UDPFromMainServer();
} 